...
(function () {
    window.dhPlayerControl = Object.assign({
        ...
        DHPlayerVersion: '',
        pkgDHPlayerVerion: [2505151714], // 配套的插件版本号
        pluginLoginInfo: {},
        isPIframe: false,
     ...
     * @desc 如果是EVS{1}, NVS{3}，NVR{6}, DVR{10}, smartNVR{14}[已弃用]，IVSS{43},需要发起通道对讲。
     */
    function getTalkType(deviceType) {
        let channelTalk = [1, 3, 6, 10, 14, 43]
    ...
    VideoPlayer.fn = VideoPlayer.prototype = {
        ...
        _update(param) {
            ...
            let { windowType, isResetConnect, request, division, visible, draggable, showBar, showBorder, shieldClass, coverShieldClass, parentIframeShieldClass, language } = param
            // 断线重连 (不对外开放，默认支持断线重连)
            ...
            // 显隐选中的窗口边框
            paramType(showBorder, 'Boolean') && !(showBorder === this.setting.showBorder) && (this.setting.showBorder = showBorder, this.setWindowBorder())
            // 窗口分割
            ...
            this.send(
                ...
                },
                {
                    onSuccess: function (data) {
                        console.log("chuangjian  chneggg ")
                        ...
                        that.setWindowBorder(that.setting.showBorder)
        ...
        setLanguage(language) {
            return new Promise((resolve, reject) => {
                this.send({
                    ...
                    info:{
                        LanguageType: language || this.setting.language || (localStorage.language === 'en' ? 'en' : 'zh') ,
                    }
                }, {
                    onSuccess: () => {
                        resolve()
        ...
        /**
         * 设置窗口边框
         * @param {*} showBorder true-显示边框 false-不显示边框
         */
        setWindowBorder: function (showBorder) {
            this.setting.showBorder = showBorder
            this.send({
                method: 'window.videoWndFrameLine',
                info: {
                    enable: this.setting.showBorder
                }
            })
        },
        /**
         ...
         * @param { Number } item.gbDevice 是否国标接入或国标级联(用于对讲)
        ...
        startReal: function (option, { isReOpen, isSubStream, count } = {}, type = 'real') {
            let tempList = []
            // 获取当前窗口分割数
            const totalWindows = this.setting.division;
            ...
            option.forEach(item => {
                // 在这里查找空闲窗口
                const freeSnum = findFreeWindow(this.setting.channelList, totalWindows);
                ...
                if (freeSnum !== -1) {
                    ...
                    let playVideo = () => {
                        ...
                        if (this.setting.usePluginLogin) {
                            ...
                            this.setting.request[type](getAjaxParam(item, type)).then(res => {
                                ...
                                // 播放失败时，将该通道从channelList中移除
                                this.setting.channelList = this.setting.channelList.filter(realItem => !(realItem.snum === item.snum && realItem.channelId === item.channelId))
                    ...
                    // 没有找到空闲窗口，这里可以添加您的处理逻辑
                    console.warn(`没有找到空闲窗口来播放通道: ${item.channelName || item.channelId}`);
                    // 例如，您可以选择跳过播放，或者覆盖某个窗口
                    // 目前我选择跳过该通道的播放
                    this.setting.realError && this.setting.realError(item, {
                        code: -1, // 可以自定义一个错误码
                        i18nKey: 'video.player.no.free.window',
                        message: '没有空闲窗口可用'
                    });
                }
            })
        ...
        startTalk: async function (snum = 0, type = 'talk') {
            ...
            this.setting.request[type]({ data: param }).then(res => {
                ...
                this.talkByUrl({
                    ...
                    gbDevice: typeof res.gbDevice !== undefined && Boolean(res.gbDevice) || Boolean(talkParam.gbDevice),
                    snum
                })
            }).catch(err => {
         ...
         * @param { Boolean } option.gbDevice 是否是国标设备
         */
         ...
         * @param { ivsType } 规则框类型 1-智能规则框，2-智能目标框 (不传默认为1) 7-辅助帧
         */
        ...
        cover: function () {
            ...
            pOutContent && shieldFn([{
                ...
                right: pOutContent.left + pOutContent.width,
                bottom: pOutContent.top + pOutContent.height
            }])
