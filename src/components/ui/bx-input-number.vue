<template>
  ...
  <div
    v-if="['rate', 'progress'].includes(subType)"
    style="height: 40px; display: flex; align-items: center; width: 100%"
  >
    <el-rate
    ...
    <!-- 进度条：不可编辑时使用progress组件，可编辑时使用slider组件 -->
    <el-slider
      v-model="currentValue"
      style="width: 100%"
      @change="handleInputChange"
  ...
  >
    <template
      slot="append"
      v-if="fieldMoreConfig.moreConfig && fieldMoreConfig.moreConfig.appendText"
      >{{ fieldMoreConfig.moreConfig.appendText }}</template
    >
    <template
      slot="prepend"
      v-if="
        fieldMoreConfig.moreConfig && fieldMoreConfig.moreConfig.prependText
      "
      >{{ fieldMoreConfig.moreConfig.prependText }}</template
    >
  </el-input>
</template>
<script>
...
export default {
  ...
  computed: {
    subType() {
      return this.fieldMoreConfig?.subType;
    },
    minDisabled() {
