  ...
  methods: {
        ...
        switch (this.fieldType) {
          ...
          case 'fkjsons':
            if (Array.isArray(obj)) {
              this.selected = obj.map(item => item[ this.valueCol ]).filter(item => item && item)
            }
            break;
          case 'fk':
    ...
    setFieldVal () {
      ...
      if (Array.isArray(this.selected)) {
        switch (this.fieldType) {
          case 'fkjsons':
            val = this.allData.filter(item => this.selected.indexOf(item[ this.valueCol ]) !== -1).map(item => {
              delete item.label;
              delete item.value;
              return item
            })
        ...
        switch (this.fieldType) {
          case 'fkjson':
            val = this.allData.find(item => this.selected.indexOf(item[ this.valueCol ]) !== -1)
        ...
        this.gridData = this.gridData.map(item => {
          if (item[ this.valueCol ] === row[ this.valueCol ]) {
            ...
            if (item.checked) {
              ...
              this.selected = this.selected.filter(e => e !== item[ this.valueCol ])
            }
          }
          return item
        })
    ...
    loadOptions (query = {}) {
        ...
        for (var condition of fmtConditions) {
          ...
          if (typeof condition.value === 'object') {
            if (condition.value[ "value_type" ] == "constant") {
              ...
              if (this.defaultValues && this.defaultValues[ condition.value[ "value_key" ] ]) {
                obj.value =
              ...
              // 主表数据
              if (this.mainformDatas && typeof this.mainformDatas === 'object' && this.mainformDatas[ condition.value[ "value_key" ] ]) {
                obj.value = this.mainformDatas[ condition.value[ "value_key" ] ] || null;
              }
            }
          }
      ...
      return this.selectList(queryJson).then(response => {
        if (response && response.data && response.data.data) {
          ...
          let allData = _.uniqBy([ ...this.gridData, ...this.allData, ..._.cloneDeep(this.selectedGridData) ], this.valueCol)
