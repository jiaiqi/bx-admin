  ...
  methods: {
    ...
    async handleCashPayment(amount) {
      ...
      try {
        const payInfoParam = await this.handleAddInfoIntoPayment();
        if (payInfoParam) {
          console.log('获取到支付参数：', payInfoParam);
          // // 测试用
          // setTimeout(() => {
          //   this.showLoading = false;
          //   this.qrcodeInfo.qrCd = 'https://www.baidu.com';
          //   this.qrcodeInfo.odrNo = 'TEST' + Date.now();
          //   this.qrcodeInfo.transAddnInfo = '测试二维码';
          //   this.isShowQrcode = true;
          //   console.log('测试二维码已生成');
          // }, 2000); // 模拟2秒的加载时间
