  ...
  <el-row class="filter-view">
    ...
    <el-row v-if="supportGroup">
      ...
      >
        <p>分组字段</p>
        <field-editor
          v-for="field in sortFields(groupFields)"
          :field="field"
          :content-fields="[field]"
          :key="field.info.name"
        >
    ...
    <el-row v-if="supportGroup">
      ...
      >
        <p>聚合字段</p>
        <field-editor
          v-for="field in sortFields(reduceFields)"
          :field="field"
          :content-fields="[field]"
          :key="field.info.name"
        >
  ...
  methods: {
    ...
    gotFlagsOptions: function (fields, complementFlagsField) {
      ...
      for (let key in fields) {
        ...
        if (
          complementFlagsField &&
          complementFlagsField.model &&
          complementFlagsField.model
            .map((item) => this.noramlizeFieldName(item))
    ...
    buildResultHeader: function () {
      ...
      if (groups && groups.length > 0) {
        return groups.map((group) => {
          let srvCol = this.srvCols.filter(
            (srvCol) => srvCol.columns == group.colName
          )[0];
          let type = group.type;
        ...
        // no group
        return this.srvCols
          .filter((srvCol) => srvCol.in_list && srvCol.columns != "id")
          .map((srvCol) => {
            return {
              label: srvCol.label,
    ...
    buildRelationConditions() {
      ...
      if (conditions?.length) {
        function extractVariables(str) {
          // // 定义正则表达式，用于匹配 ${} 中的内容
          // const regex = /\$\{([^}]*)\}/g;
          // 定义正则表达式，用于匹配 ${} 中的内容，并忽略前缀 "row."
          const regex = /\$\{row\.?([^}]*)\}/g;
          let matches;
          const variables = [];

          // 使用正则表达式的 exec 方法循环匹配所有符合模式的字符串
        ...
        const relationConditions = {
          relation: "AND",
          data: conditions.reduce((acc, cur) => {
            const curCol = this.allFields[cur.colName]?.info;
            const formatDisplay = curCol?.moreConfig?.format_display;
            if (formatDisplay) {
              let arr = extractVariables(formatDisplay);
    ...
    buildConditions: function (groups, headers) {
      ...
      for (let key in fields) {
        let inSrvCols =
          this.srvCols.filter((item) => item.columns === key).length > 0;
        let field = fields[key];
        if (!inSrvCols) {
    ...
    decorateFields: function () {
      ...
      for (let key in this.fields) {
        ...
        if (fieldInfo.isFinder()) {
          ...
          let hasDispCol =
            fieldArr.filter(
              (item) => item.info.name === "_" + fieldInfo.name + "_disp"
            ).length > 0;
          if (hasDispCol) {
            toRemove.add(key);
          }
        ...
        } else if (['MonthRange', 'Date', 'Time', 'DateTime'].includes(fieldInfo.type)) {
          field.ruleType = "between";
        } else if (fieldInfo.type == "Enum" || fieldInfo.type == "Dict") {
          field.ruleType = "in";
          field.model = [];
        } else if (
          fieldInfo.isJoinedDisp() ||
          fieldInfo.isTextual() ||
          fieldInfo.isFinder() ||
    ...
    resetFFlagsField() {
      let inCondFields = Object.keys(this.fields).filter((fieldName) => {
    ...
    createReduceFields: function () {
      ...
      for (let i in this.srvCols) {
        ...
        if (
          ...
          (fi.name.startsWith("_") && fi.name.endsWith("_disp"))
        ) {
          continue;
        }
        ...
        if (fi.isTemporal()) {
          ...
          f.options = commonReduceOptions
            .concat(comparableReduceOptions)
            .concat(numbericReduceOptions);
          f.model.push("avg");
        } else {
          f.options = commonReduceOptions;
          f.model.push("count");
      ...
      this.rflagsField.optionsFunc = (_) =>
        this.gotFlagsOptions(this.reduceFields, this.gflagsField);
    },
    ...
    createGroupFields: function () {
      ...
      this.gflagsField.optionsFunc = (_) =>
        this.gotFlagsOptions(this.groupFields, this.rflagsField);
    },
