<template>
  <div>
    ...
    <el-table :data="tableData" height="800" border class="tb-edit" style="width: 100%" highlight-current-row
      @row-click="handleCurrentChange">

      <el-table-column type="selection" label="全选" header-align="left" width="50px">
      </el-table-column>

      <el-table-column :prop="data.columns" :fixed="getfixedValue(data.columns)"
        :label="data.label === '商品条码' ? '商品数量' : data.label" v-for="(data, index) in tableHeadr" :key="index"
        :width="data.col_type == 'Integer' ? 90 : 180">
        <template slot-scope="scope">
          <div v-if="is_edit">
            ...
            <el-select size="small" v-if="data.col_type === 'Enum' || data.col_type === 'Dict'"
              v-model="scope.row[data.columns].value" placeholder="请选择" @change="handleEdit(scope.$index, scope.row)">>
              <el-option v-for="item in data.option_list_v2" :key="item.value" :label="item.key"
                :value="item.value"></el-option>
            </el-select>
            <el-date-picker v-else-if="data.col_type === 'datetime'" v-model="scope.row[data.columns].value"
              type="datetime" clearable placeholder="选择日期时间" @change="handleEdit(scope.$index, scope.row)">
            </el-date-picker>
            ...
            <el-input size="small" v-else v-model="scope.row[data.columns].value"
              @change="handleEdit(scope.$index, scope.row)"></el-input>
          </div>
          <div v-if="!is_edit">
            <el-input size="small" v-if="data.columns === 'goods_code'" v-model="scope.row[data.columns].value"
              @change="handleEdit(scope.$index, scope.row)"></el-input>
            <el-input-number size="small" v-else-if="data.columns === 'goods_type'"
              v-model="scope.row[data.columns].value" @change="handleEdit(scope.$index, scope.row)" :min="1"
              label=""></el-input-number>
            <span v-else>{{ scope.row[data.columns].value }}</span>
          </div>
        </template>
      </el-table-column>
