...
// import '../../static/langs/zh_CN.js' // 本地汉化
...
export default {
  ...
  data () {
    return {
      //初始化配置
      init: {
        language_url: '../../static/langs/zh_CN.js', // 这里需要单独处理
        language: 'zh_CN',
        ...
        images_upload_handler: (blobInfo, success, failure) => {
          ...
          that.percentage = 0
          let uploadUrl = request.baseURL + `/api/server/publish/uploadImg`
  ...
  methods: {
    ...
    async uploadFile (file, folder = 'images') {
      ...
      const { data } = await axios({
        method: 'POST',
        url: request.baseURL + `/api/server/publish/uploadvedio`,
        data: formData,
    ...
    async uploadImg (file, folder = 'images') {
      ...
      const { data } = await axios({
        method: 'POST',
        url: request.baseURL + `/api/server/publish/uploadImg`,
        data: file,
        headers: { 'Content-Type': 'multipart/form-data' }
      })
