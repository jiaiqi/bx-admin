 ...
 computed: {
  ...
  data() {
    ...
    return {
      ...
      init: {
        ...
        content_style: 'body {font-size: 14pt; font-family:Arial;text-indent: 2em;}',
  ...
  methods: {
    ...
    recoverFileAddress(val){
      // 替换文件前缀
      return val?.replaceAll?.('$bxFileAddress$',this.serviceApi().downloadFile)||''
    },
    replaceFileAddressSuffix(val){
      return val?.replaceAll?.(this.serviceApi().downloadFile,'$bxFileAddress$')||''
    },
    setSrvVal(val){
      // 设置值
    ...
    onChange(){
      ...
      this.$emit('field-value-changed',this.replaceFileAddressSuffix(this.myValue))
    },
    onClick(e) {
    ...
    setup: function (editor) {
      ...
      editor.ui.registry.addButton('customlayout', {
        ...
        onAction: function () {
          ...
          that.$emit('field-value-changed', this.replaceFileAddressSuffix(div.innerHTML))
        }
      });
