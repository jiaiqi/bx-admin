...
export default {
    ...
    methods: {
                       ...
                       await this.$axios
                            ...
                            .then((res) => {
                                ...
                                if (item.option_list_v2.is_tree) {
                                    ...
                                    this[`${service}V2`].storeData[`${item.columns}`].array = treeArray;

                                } else {
                                    console.log(this,"什么是storeData")
                                    this[`${service}V2`].storeData[`${item.columns}`].array = data;
       ...
       async  getArrayList(app,service,filterColumn) {
            let columnArray;
            let appName; 
            ...
            if(this[`${service}V2`]){
                ...
                appName = this[`${service}V2`].gridButton[0].application;
                 ...
                 await this.getV222222Columns(app,service);
                 columnArray = this[`${service}V2`].srv_cols;
                 appName = this[`${service}V2`].gridButton[0].application;
            }
        ...
        async getV222222Columns(app,service) {
            let condition = {
            ...
            let returnData = await this.$axios.post(
                `${app}/select/srvsys_service_columnex_v2_select?colsel_v2=${this.serviceName}`,
                condition
            );
