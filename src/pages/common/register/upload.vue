<template>
  <div class="d-flex w-100">
    <div v-if="Array.isArray(files) && files.length > 0" class=" d-flex">
      <div v-for="(file, index) in files" class="border mr-1"
        :style="`background:#f2f2f2;width:${mainProps.width}px;height:${mainProps.height}px;position: relative;`">
        <b-img class="p-1" v-bind="mainProps" :src="file['_url']" alt="加载失败"> </b-img>
        <div style="height:28px;width:28px;background-color:#0000006b;position: absolute;right: 0;top:0;"
          class="float-right d-flex justify-content-center align-items-center text-white cursor-pointer" @click="del(file)">
          <b-icon-x></b-icon-x>
        </div>
      </div>
    </div>
    <div v-if="!isMax" class="block d-flex justify-content-center align-items-center h2 m-0"
      :style="`background:#f2f2f2;width:${mainProps.width}px;height:${mainProps.height}px;`" @click="onCheck">
      <input :name="initColumn" type="file" multiple="multiple" style="display: none;" accept="image/*"
        :ref="`uploadInput${initColumn}`" @change="getImg">
      ...
      if(res?.data?.state==='SUCCESS'){
        ...
        if(res?.data?.data){
          ...
          this.files = resData.map(item => {
            item['_url'] = `${this.hostPath}/file/download?filePath=${item.fileurl}`
            if (this.getToken) {
              item['_url'] = `${item['_url']}&bx_auth_ticket=${this.getToken}`
            }
            return item
          })
        } else {
