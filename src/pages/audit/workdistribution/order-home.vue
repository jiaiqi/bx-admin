    ...
    <div class="work_tab">
      <div class="work_tab_cot">
        <div
          ...
          :key="item.code"
          :class="activeIndex === index ? 'act_tab_row' : 'tab_row'"
    ...
    setTab(item, index) {
      ...
      if (item.code !== 1) {
        ...
        if (this.initialRouteInfo && Object.keys(this.initialRouteInfo).length > 0) {
          this.$router.push({ name: item.path, query: { ...this.initialRouteInfo, pageType: this.pageType || undefined } });
        } else {
          const storedInfo = sessionStorage.getItem("oderInfo");
          if (storedInfo) {
            try {
              const parsedInfo = JSON.parse(storedInfo);
              this.$router.push({ name: item.path, query: { operate_params: storedInfo, pageType: this.pageType || undefined } });
            } catch (e) {
              console.error('解析sessionStorage数据失败：', e);
              this.$router.push({ name: item.path, query: { pageType: this.pageType || undefined } });
            }
          } else {
            this.$router.push({ name: item.path, query: { pageType: this.pageType || undefined } });
          }
