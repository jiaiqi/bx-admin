    ...
    export default {
      ...
      computed: {
        comColMapRun: function() {
          let json = this.pageItem?.com_srv_col_map_json || this.pageItem?.cols_map_json
      ...
      methods: {
        ...
        onClickCell(cell) {
          ...
          if (cell && cell.cellsLayout && cell.cellsLayout.jump_json) {
            ...
            const colsMapJson = cell?.cellsLayout?.jump_json?.cols_map_json?.cols_map_detail_json || []
            if (colsMapJson?.length) {
              ...
              colsMapJson.forEach(item => {
                if (item.to_type === 'URL' && item.from_type === '当前数据' && data[item.col_from]) {
                  path += `&${item.col_to}=${data[item.col_from]}`
                } else if (item.to_type === 'URL' && item.from_type === '页面' && this.componentParamsModels[item
                    .col_from]) {
                  path += `&${item.col_to}=${this.componentParamsModels[item.col_from]}`
                } else if (item.to_type === 'URL' && item.from_type !== '当前数据' && item.col_from && item.col_to) {
