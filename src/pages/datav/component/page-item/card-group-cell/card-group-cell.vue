      ...
      <template
        ...
        <div v-for="(cellLayoutJson,i) in cellsLayout" :key="i" class="bx-card-cell"
          ...
          <div v-for="(item,n) in cellLayoutJson.parts_json" :key="n">
            ...
            <div v-else-if="item.parts_type == 'variable'  && partsShow(item,comColMap,cellItemData)"
              :class="'bx-cell-'+item.parts_type" @click.stop="onClickSubBlock(cellItemData,item,cellLayoutJson)"
              :style="[buildColStyleJson(item.style_json)]">
              {{getPartModelData(item,comColMap,cellItemData)}}
            </div>
            <el-image v-else-if="item.parts_type == 'iconImg'  && partsShow(item,comColMap,cellItemData)"
            ...
            <el-rate :disabled="true" v-else-if="item.parts_type == 'rate'" :count="5"
              :value="Number(getPartModelData(item,comColMap,cellItemData))||0"></el-rate>
            <el-progress :show-text="false"  :style="[buildColStyleJson(item.style_json || null)]" v-else-if="item.parts_type == 'progress'" :count="5" :define-back-color="buildColStyleJson(item.style_json || null)['background-color']||''" :color="buildColStyleJson(item.style_json || null).color||'#2979ff'" :percentage="Number(getPartModelData(item,comColMap,cellItemData))||0"></el-progress>
            ...
            <div :class="'bx-cell-'+item.parts_type"
              v-else-if="(item.parts_type == 'row' || item.parts_type == 'block')  && partsShow(item,comColMap,cellItemData) && item.hasOwnProperty('sub_card_parts_json') && item.sub_card_parts_json.length > 0"
              @click.stop="onClickSubBlock(cellItemData,item,cellLayoutJson)"
              :style="[buildColStyleJson(item.style_json || null)]">
              <div v-for="(subCol,subindex) in item.sub_card_parts_json" :key="subindex">
                <div v-if="subCol.parts_type == 'string' && partsShow(subCol,comColMap,cellItemData)"
                ...
                <div v-else-if="subCol.parts_type == 'variable' && partsShow(subCol,comColMap,cellItemData)"
                  :class="'bx-cell-'+subCol.parts_type"
                  @click.stop="onClickSubBlock(cellItemData,subCol,cellLayoutJson,item)"
                  :style="[buildColStyleJson(subCol.style_json || null)]">
                  {{getPartModelData(subCol,comColMap,cellItemData)}}
                </div>
                <el-image v-else-if="subCol.parts_type == 'iconImg' && partsShow(subCol,comColMap,cellItemData)"
                  img-mode="aspectFill" @click.stop="onClickSubBlock(cellItemData,subCol,cellLayoutJson,item)"
                ...
                <el-rate :disabled="true" v-else-if="subCol.parts_type == 'rate'" :count="5"
                  :value="Number(getPartModelData(subCol,comColMap,cellItemData))||0"></el-rate>
                <el-progress :show-text="false"  :style="[buildColStyleJson(subCol.style_json || null)]" v-else-if="subCol.parts_type == 'progress'" :count="5" :define-back-color="buildColStyleJson(subCol.style_json || null)['background-color']||''"  :color="buildColStyleJson(subCol.style_json || null).color||'#2979ff'" :percentage="Number(getPartModelData(subCol,comColMap,cellItemData))||0"></el-progress>
                ...
                <u-parse v-else-if="subCol.parts_type == '富文本' && partsShow(subCol,comColMap,cellItemData)"
                  :style="[buildColStyleJson(subCol.style_json || null)]"
                  :html="getPartModelData(subCol,comColMap,cellItemData)"></u-parse>
                <div :class="'bx-cell-'+subCol.parts_type"
                  ...
                  <template v-for="(ssubCol,ssubindex) in subCol.sub_card_parts_json">
                    ...
                    <div v-else-if="ssubCol.parts_type == 'variable' && partsShow(ssubCol,comColMap,cellItemData)"
                      :class="'bx-cell-'+ssubCol.parts_type"
                      @click.stop="onClickSubBlock(cellItemData,ssubCol,cellLayoutJson,subCol)"
                      :style="[buildColStyleJson(ssubCol.style_json || null)]">
                      {{getPartModelData(ssubCol,comColMap,cellItemData)}}
                    </div>
                    <el-image v-else-if="ssubCol.parts_type == 'iconImg' && partsShow(ssubCol,comColMap,cellItemData)"
                      img-mode="aspectFill"
                      @click.stop="onClickSubBlock(cellItemData,ssubCol,cellLayoutJson,subCol)"
                    ...
                    <el-rate :disabled="true" v-else-if="ssubCol.parts_type == 'rate'" :count="5"
                      :value="Number(getPartModelData(ssubCol,comColMap,cellItemData))||0"></el-rate>
                    <el-progress :show-text="false"   :style="[buildColStyleJson(ssubCol.style_json || null)]" v-else-if="ssubCol.parts_type == 'progress'" :count="5" :define-back-color="buildColStyleJson(ssubCol.style_json || null)['background-color']||''"   :color="buildColStyleJson(ssubCol.style_json || null).color||'#2979ff'" :percentage="Number(getPartModelData(ssubCol,comColMap,cellItemData))||0"></el-progress>
                    ...
                    <i v-else-if="ssubCol.parts_type == 'icon' && partsShow(ssubCol,comColMap,cellItemData)"
                      :class="ssubCol.parts_text" :style="[buildColStyleJson(ssubCol.style_json || null)]"
                      @click.stop="onClickSubBlock(cellItemData,ssubCol,cellLayoutJson,subCol)"></i>
                    <div :class="'bx-cell-'+ssubCol.parts_type"
                      v-else-if="(ssubCol.parts_type == 'row' || ssubCol.parts_type == 'block') && partsShow(ssubCol,comColMap,cellItemData) && ssubCol.hasOwnProperty('sub_card_parts_json') && ssubCol.sub_card_parts_json.length > 0"
                      :style="[buildColStyleJson(ssubCol.style_json || null)]"
                      @click.stop="onClickSubBlock(cellItemData,ssubCol,cellLayoutJson,subCol)">
                      <div v-for="(sssubCol,sssubindex) in ssubCol.sub_card_parts_json" :key="sssubindex">
                        <div v-if="sssubCol.parts_type == 'string' && partsShow(sssubCol,comColMap,cellItemData)"
                          :class="'bx-cell-'+sssubCol.parts_type"
                          @click.stop="onClickSubBlock(cellItemData,sssubCol,cellLayoutJson,ssubCol)"
                          :style="[buildColStyleJson(sssubCol.style_json || null)]">
                          {{getPartModelData(sssubCol,comColMap,cellItemData)}}
                        </div>
                        <div v-else-if="sssubCol.parts_type == 'variable' && partsShow(sssubCol,comColMap,cellItemData)"
                          :class="'bx-cell-'+sssubCol.parts_type"
                          @click.stop="onClickSubBlock(cellItemData,sssubCol,cellLayoutJson,ssubCol)"
                          :style="[buildColStyleJson(sssubCol.style_json || null)]">
                          {{getPartModelData(sssubCol,comColMap,cellItemData)}}
                        ...
                        <el-image
                          v-else-if="sssubCol.parts_type == 'iconImg' && partsShow(sssubCol,comColMap,cellItemData)"
                          img-mode="aspectFill"
                          @click.stop="onClickSubBlock(cellItemData,sssubCol,cellLayoutJson,ssubCol)"
                        ...
                        <el-rate :disabled="true" v-else-if="sssubCol.parts_type == 'rate'" :count="5"
                          :value="Number(getPartModelData(sssubCol,comColMap,cellItemData))||0"></el-rate>
                        <el-progress :show-text="false"  :style="[buildColStyleJson(sssubCol.style_json || null)]" v-else-if="sssubCol.parts_type == 'progress'" :count="5" :define-back-color="buildColStyleJson(sssubCol.style_json || null)['background-color']||''"   :color="buildColStyleJson(sssubCol.style_json || null).color||'#2979ff'" :percentage="Number(getPartModelData(sssubCol,comColMap,cellItemData))||0"></el-progress>
                        ...
                        <u-parse v-else-if="sssubCol.parts_type == '富文本' && partsShow(sssubCol,comColMap,cellItemData)"
                          v-show="partsShow(sssubCol,comColMap,cellItemData)"
                          :style="[buildColStyleJson(sssubCol.style_json || null)]"
                          :html="getPartModelData(sssubCol,comColMap,cellItemData)" @click="false"></u-parse>
                        <div :class="'bx-cell-'+sssubCol.parts_type"
                          v-else-if="(sssubCol.parts_type == 'row' || sssubCol.parts_type == 'block') && partsShow(sssubCol,comColMap,cellItemData) && sssubCol.hasOwnProperty('sub_card_parts_json') && sssubCol.sub_card_parts_json.length > 0"
                          :style="[buildColStyleJson(ssubCol.style_json || null)]"
                          @click.stop="onClickSubBlock(cellItemData,sssubCol,cellLayoutJson,ssubCol)">
  
                          <div v-for="(sssubCol4,sssub4index) in sssubCol.sub_card_parts_json" :key="sssub4index">
                            <div v-if="sssubCol4.parts_type == 'string' && partsShow(sssubCol4,comColMap,cellItemData)"
                              :class="'bx-cell-'+sssubCol4.parts_type"
                              @click.stop="onClickSubBlock(cellItemData,sssubCol4,cellLayoutJson,sssubCol)"
                              :style="[buildColStyleJson(sssubCol4.style_json || null)]">
                              {{getPartModelData(sssubCol4,comColMap,cellItemData)}}
                            </div>
                            <div
                              v-else-if="sssubCol4.parts_type == 'variable' && partsShow(sssubCol4,comColMap,cellItemData)"
                              :class="'bx-cell-'+sssubCol4.parts_type"
                              @click.stop="onClickSubBlock(cellItemData,sssubCol4,cellLayoutJson,sssubCol) "
                              :style="[buildColStyleJson(sssubCol4.style_json || null)]">
                              {{getPartModelData(sssubCol4,comColMap,cellItemData)}}
                            ...
                            <el-image
                              v-else-if="sssubCol4.parts_type == 'iconImg' && partsShow(sssubCol4,comColMap,cellItemData)"
                              img-mode="aspectFill"
                              @click.stop="onClickSubBlock(cellItemData,sssubCol4,cellLayoutJson,sssubCol)"
                            ...
                            <el-rate :disabled="true" v-else-if="sssubCol4.parts_type == 'rate'" :count="5"
                              :value="Number(getPartModelData(sssubCol4,comColMap,cellItemData))||0"></el-rate>
                            <el-progress :show-text="false"   :style="[buildColStyleJson(sssubCol4.style_json || null)]" v-else-if="sssubCol4.parts_type == 'progress'" :count="5"  :define-back-color="buildColStyleJson(sssubCol4.style_json || null)['background-color']||''"  :color="buildColStyleJson(sssubCol4.style_json || null).color||'#2979ff'" :percentage="Number(getPartModelData(sssubCol4,comColMap,cellItemData))||0"></el-progress>
                            <i
                              ...
                              @click.stop="onClickSubBlock(cellItemData,sssubCol4,cellLayoutJson,sssubCol) "></i>
                            <u-parse
                              v-else-if="sssubCol4.parts_type == '富文本' && partsShow(sssubCol4,comColMap,cellItemData)"
                              v-show="partsShow(sssubCol4,comColMap,cellItemData)"
                              :style="[buildColStyleJson(sssubCol4.style_json || null)]"
                              :html="getPartModelData(sssubCol4,comColMap,cellItemData)" @click="false"></u-parse>
                          </div>
                        </div>
                      </div>
                    </div>
    ...
    export default {
      ...
      computed: {
        showRowButtons() {
          ...
          if (this.pageItem && this.pageItem.com_type == 'list') {
            if (this.pageItem.list_json.list_options && this.pageItem.list_json.list_options.indexOf('单元按钮') !== -
              1 &&
        ...
        cellStyle: function() {
          let style = this.cardLayout && this.cardLayout?.style_json ? this.cardLayout?.style_json : {}
          return style
        },
        totalMaximum: function() {
          ...
          let maximum = config && config.rows_max && config.cols_num ? config['rows_max'] * config['cols_num'] :
            maxPageRowNumber
        ...
        gridStyle: function() {
          ...
          let height = config.hasOwnProperty('style_json') && config.style_json.hasOwnProperty('height') ? config
            .style_json.height : 'auto'
        ...
        srvApp() {
          return this.pageItem?.srv_req_json?.mapp || uni.getStorageSync('activeApp')
        },
      },
      created() {
        // self = this
      ...
      methods: {
        ...
        getButtonVisible(cellItemData, button, ib) {
          ...
          if (Array.isArray(cellItemData?._buttons) && cellItemData._buttons.length >= ib + 1) {
            return cellItemData['_buttons'][ib] === 1
          } else {
            return false
            console.log(cellItemData, 'getButtonVisible---->cellItemDatacellItemDatacellItemData');
            return button.permission && (!button.hasOwnProperty('visible') || button?.visible === '是')
          }
        },
        onRowButton(e) {
          if (this.readOnly) {
          ...
          if (item && itemData && !!map) {
            ...
            switch (optionsType) {
              ...
              case '地图导航':
                ...
                val = null
                val = {}
                if (lgtKey && map.hasOwnProperty(lgtKey) && itemData.hasOwnProperty(map[lgtKey]) && itemData[
                    map[lgtKey]]) {
                  val['lgt'] = itemData[map[lgtKey]]
                } else {
                  val = null
                }
                if (latKey && map.hasOwnProperty(latKey) && itemData.hasOwnProperty(map[latKey]) && itemData[
                    map[latKey]]) {
                ...
                break;
              default:
                if (item.hasOwnProperty('variable') && key && map.hasOwnProperty(key) && itemData.hasOwnProperty(map[
                    key]) && itemData[map[key]]) {
                  val = itemData[map[key]] || ''
                } else if (item.hasOwnProperty('variable') && key && itemData.hasOwnProperty(key) && itemData[key]) {
                  val = itemData[key] || ''
            ...
            if (item.hasOwnProperty('variable') && key && itemData.hasOwnProperty(key) && itemData[key]) {
              val = itemData[key]
            } else if (item.parts_type === 'string' && item.parts_text) {
              val = this.renderStr(item.parts_text, {
                data: itemData,
