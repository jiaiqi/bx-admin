...
export default {
  ...
  methods: {
    ...
    setOptions4Cartesian: function () {
      if (
        !this.series ||
        this.series.filter((item) => item.visible === "是").length == 0
      ) {
        return;
      }
      ...
      this.echartOptions.grid = this.grids.map((item) => {
        let grid = {};
        if (isNumber(item.grid_top)) {
          grid.top = item.grid_top + "%";
        }
        if (isNumber(item.grid_left)) {
          grid.left = item.grid_left + "%";
        }
        if (isNumber(item.grid_right)) {
          grid.width = item.grid_right - item.grid_left + "%";
        }
        if (isNumber(item.grid_bottom)) {
      ...
      let xaxes = this.axes
        .filter((item) => item.xory == "X轴")
        .map((item) => {
          let xaxisType = this.axisTypeMap[item.axis_type];
          let gridIndex = this.grids.findIndex(
            (e) => e.grid_no === item.grid_no
          );
          return {
            type: xaxisType,
            gridIndex: gridIndex,
      ...
      this.series
        .filter((item) => item.visible === "是")
        .map((item) => {
          let yAxisIndex = yaxes.findIndex(
            (axis) => axis.axis_no == item.yaxis_no
          );
          let xAxisIndex = xaxes.findIndex(
            (axis) => axis.axis_no == item.xaxis_no
          );
          let xAxis = this.axes.find((axis) => axis.axis_no == item.xaxis_no);
    ...
    setOptions4Pie: function () {
      this.pieCharts
        .filter((item) => item.visible === "是")
        .map((item) => {
          let xDimIndex = this.headers.findIndex((h) => h === item.x_dim);
          let yDimIndex = this.headers.findIndex((h) => h === item.y_dim);
          let seriesData = [];
          ...
          let defaultR = 55;
          let r =
            item.grid_right && item.grid_left
              ? (item.grid_right - item.grid_left) / 2
              : defaultR;
          let radius = r + "%";
    ...
    loadData: function () {
      ...
      return this.select("srvsys_stat_chart_select", chartConditions)
        .then((response) => {
          ...
          this.condition =
            chart.conditions && chart.conditions.trim().length > 0
              ? eval(chart.conditions)
              : [];
          this.group =
            chart.groups && chart.groups.trim().length > 0
              ? eval(chart.groups)
              : null;
          this.order =
            chart.orders && chart.orders.trim().length > 0
              ? JSON.parse(chart.orders)
              : null;
        })
        .then((_) => {
          return this.loadColsV2(this.service, "list").then((response) => {
            if (
              !response ||
              !response.data ||
              !response.data.data ||
