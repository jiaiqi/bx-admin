  ...
  <div style="" v-loading="loading" class="retail-tree">
    ...
    <el-form :model="roleModel" :rules="rules" ref="roleForm" label-width="100px" class="demo-ruleForm">
      <el-form-item label="角色名称" prop="role_name">
        <el-input :disabled="pageType == 'detail'" v-model="roleModel.role_name" placeholder="请输入角色名称"></el-input>
      </el-form-item>
      <el-form-item label="角色描述" prop="remark">
        <el-input :disabled="pageType == 'detail'" type="textarea" v-model="roleModel.remark"
          placeholder="请输入角色描述"></el-input>
      </el-form-item>
    </el-form>
    <el-form ref="form" :model="terminalTypeModel" label-width="100px">
      <el-form-item :label="item.terminal_type == '一体机' ? '台式收银机' : item.terminal_type"
        v-for="(item, index) in terminalType" :key="index">
        <el-checkbox v-model="cfg.checked" v-if="pageType !== 'detail'" v-for="(cfg, n) in item.checkCfg" :key="n"
          @change="checkChange(cfg, item, index)">{{ cfg.label }}</el-checkbox>
        ...
        <el-tree :ref="'tree' + index" :data="item._treeData" show-checkbox :node-key="nodeKey"
          :default-expand-all="getUserChecked(item.checkCfg, 'expand')" :check-strictly="false"
          :default-checked-keys="item.checkedKeys" @current-change="currentChange" @check="treeCurrentChange"
          :props="defaultProps">
...
export default {
    ...
    return {
      ...
      defaultProps: {
        disabled: function (data, node) {
          ...
          if (data.is_require_srv == '是' && node.hasOwnProperty('parent') && node.parent.hasOwnProperty('checked') && node.parent.checked == true && node.checked == true || data.disabled) {
            return true
          } else {
            return false
          }
  ...
  computed: {
    ...
    roleNo: function () {
      return this.$route.query.role_no || this.$route.params.role_no || 'RL_202210270003'
    }
  },
  methods: {
    onSubmit() {
    ...
    treeCurrentChange(checkedNodes, checkedKeys) {
        ...
        if (node.data[self.nodeKey] == checkedNodes[self.nodeKey]) {
          ...
          for (let child of childNodes) {
            if (child.data.is_require_srv == '是') {
              console.log('child.data.is_require_srv', child.data.is_require_srv, child.data.fun_name)
      ...
      console.log('treeCurrentChange', type, ref, checkedNodes, checkedKeys, allKeys, checked)
    },
    ...
    getNodeDisabled: function (data, node) {
      console.log(data, node)
      if (data.is_require_srv == '是' && node.hasOwnProperty('parent') && node.parent.hasOwnProperty('checked') && node.parent.checked == true) {
        return true
      } else {
        return false
      }
