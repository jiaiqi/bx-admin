...
export default {
  ...
  methods: {
    ...
    initEchats2() {
      console.log(
        this.$refs.echarts2,
        "===this.$refs.echarts2==this.$refs.echarts2"
      );
      this.myCharts1 = this.$echarts.init(this.$refs.echarts2);
    ...
    initEchats3() {
      ...
      this.myCharts1 = this.$echarts.init(this.$refs.echarts3);
      this.options = {
        title: {
          text: "传感器A（门口）", //图表顶部的标题
          show: true,
      ...
      this.myCharts1.setOption(this.options);
    },
    getV2TableInfo(url) {},
    ...
    getMainTableInfo(sucessData) {
      ...
      if (this.tableInfo.tableClassify) {
        ...
        sucessData.data.data.forEach((item, index) => {
          ...
          if (!classifyArray[`${key}`]) {
            ...
            classifyArray[`${key}`];
            echarts.title.text = this.getTemplateInfo(
              echarts.title.text
            ).setColumns({
              title: key,
            });
            echarts.uuid = this.getUuid();
            this.getEchats(echarts, item);

            classifyArray[`${key}`] = echarts;
          } else {
            this.getEchats(classifyArray[`${key}`], item);
          }
        });
      ...
      this.$nextTick(() => {
        console.log(
          this.classifyArray,
          "--this.classifyArray--this.classifyArray"
        );
        for (let key in this.classifyArray) {
          console.log(this.$refs, "---------$refs-------$refs---$refs");
          let echartsKey = `echarts${this.classifyArray[key].uuid}`;
          console.log(echartsKey, "--echartsKey-");

          setTimeout(() => {
            let dom = this.$refs[`${echartsKey}`][0];
            console.log(dom, "--dom--3321321-");
            let myCharts1 = this.$echarts.init(dom);
            console.log(myCharts1, "---myCharts1--");
            console.log(
              this.classifyArray[key],
              "--this.classifyArray[key]---this.classifyArray[key]--"
            );
            let options = this.classifyArray[key];
            options.series.forEach((item) => {
              item.data = item.data.map((itemX) => {
