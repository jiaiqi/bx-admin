...
/**
 * @namespace BMapGL鐨勬墍鏈塴ibrary绫诲潎鏀惧湪BMapLib鍛藉悕绌洪棿涓�
 */
...
(function () {
    ...
    var LuShu =
        ...
        BMapLib.LuShu = function (map, path, opts) {
            ...
            //杩涜鍧愭爣杞
            this.smallPoints = [path[0]];
            let maptype=this._map.getMapType();
            // console.log(maptype,"==maptype==maptype=maptype=maptype")
    ...
    LuShu.prototype.addPoint = function (point) {
        if (this._path) {
    ...
    LuShu.prototype.addPoints = function (points) {
        console.log(window.intervals, "IntervalInterval")
        if (this._path) {
            this._path = this._path.concat(points);
            if (this.isEnd) {
                this.isEnd = false;
                this._moveNext(this.i);
            }
    ...
    LuShu.prototype.start = function () {
        ...
        LuShu.prototype.stop = function () {
            ...
            this._addMarker();
            this._map.setCenter(this._path[0]);

            setTimeout(function () {
                _this._addInfoWin();
                _this._marker.setPosition(_this._path[0]);

            }, 200);

            this.smallPoints = [this._path[0]];
    ...
    baidu.object.extend(LuShu.prototype, {
         ...
         * @return 鏃犺繑鍥炲€�.
         */
        _move: function (initPos1, targetPos1, effect) {
            ...
            if(me.startSs){
                timer=200;
                step=me.timeGap*1000*60/timer

            }else{
                step = this._opts.speed / (1000 / timer);
            }
            var step = this._opts.speed / (1000 / timer);
            console.log(initPos1,"==initPos1==")
            var init_pos = me._map.pointToPixel(initPos1);
            console.log(init_pos,"==init_pos==init_pos==init_pos=")
            var target_pos = me._map.pointToPixel(targetPos1);
            ...
            if (count < 1) {
                ...
                me.smallPoints.push(targetPos1);

                if (me._opts.autoView) {
                    if (!me._map.getBounds().containsPoint(targetPos1)) {
                        me._map.setCenter(targetPos1);
                    }
                }

                var lineArray = [];
                lineArray.push(me.smallPoints[me.smallPoints.length - 2]);
                lineArray.push(me.smallPoints[me.smallPoints.length - 1]);

                me._marker.setPosition(targetPos1);
                var line = new BMapGL.Polyline(lineArray, {
                    strokeColor: 'red',
                    strokeWeight: 4,
                    strokeOpacity: 1
                });
    ...
    baidu.object.extend(CustomOverlay.prototype, {
        ...
        setPosition: function (poi, markerSize) {
            ...
            overlayW = parseInt(this._div.clientWidth || styleW, 10),
                ...
                console.log(px,"--pxpxppxpx-")
            // console.log(poi,"--p456oi-")
